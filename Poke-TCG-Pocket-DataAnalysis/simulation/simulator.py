...
def take_turns(game: Game, debug:bool=False) -> int:
    turn_count = 0
    ...
    while True:
        if debug:
            game.print()
            print(f"\nTurn {turn_count + 1}")
        game_control.automated_take_turn(game, debug=debug)
...
def simulate(evo_1: int, evo_2: int, debug:bool=False) -> (int | None):
    ...
    winner = take_turns(game, debug=debug)
    if debug:
        if not winner is None:
